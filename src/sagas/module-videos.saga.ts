...
export class ModuleVideosSagas {
  ...
  @Saga()
  moduleVideoUpdated = (events$: Observable<any>): Observable<ICommand> => {
    return events$.pipe(
      ofType(ModuleVideoUpdatedEvent),
      delay(500),
      map((event) => {
        this.logger.log(
          'Inside [ModuleVideoUpdatedEvent] Saga for example send a email'
        );
        this.logger.log(
          'Inside [ModuleVideoUpdatedEvent] Saga event moduleVideoUpdated:' +
            JSON.stringify(
              hiddenPrivateDataFromObject(event, DEFAULT_PRIVATE_DATA)
            )
        );
