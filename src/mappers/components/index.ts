...
export class ComponentMapper {
  toWhereFindAllPrisma(where: IWhere<WhereDataFindAllComponentsDTO>) {
  ...
  toFindAllPrisma(data: FindAllComponentsDTO) {
    const prismaMapper = new PrismaMapper();
  ...
  toFindAllQueryClean(query: FindAllComponentsQuery): FindAllComponentsQuery {
    if (!query) {
      return null;
    }
    return {
  ...
  toModel(component: ComponentEntity) {
    return component && new ComponentModel(component);
  }
  toModels(components: ComponentEntity[]) {
    return components?.length > 0
      ? components.map(this.toModel).filter(Boolean)
      : [];
  }
}
