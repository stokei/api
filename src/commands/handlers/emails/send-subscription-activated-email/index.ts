...
type SendSubscriptionActivatedEmailCommandKeys =
  keyof SendSubscriptionActivatedEmailCommand;

@CommandHandler(SendSubscriptionActivatedEmailCommand)
export class SendSubscriptionActivatedEmailCommandHandler
  implements ICommandHandler<SendSubscriptionActivatedEmailCommand>
{
  private readonly logger = new Logger(
    SendSubscriptionActivatedEmailCommandHandler.name
  );
  constructor(
  ...
  async execute(command: SendSubscriptionActivatedEmailCommand) {
    ...
    try {
      ...
      if (!data?.app) {
        throw new ParamNotFoundException<SendSubscriptionActivatedEmailCommandKeys>(
          'app'
        );
      }
      if (!data?.toAccount) {
      ...
      return await this.sendEmailService.execute({
        route: '/emails/subscription-activated',
        to: {
          name: toAccount.name,
  ...
  private clearData(
    command: SendSubscriptionActivatedEmailCommand
  ): SendSubscriptionActivatedEmailCommand {
    return cleanObject({
