...
export class PluginMapper {
  toWhereFindAllPrisma(where: IWhere<WhereDataFindAllPluginsDTO>) {
  ...
  toFindAllPrisma(data: FindAllPluginsDTO) {
    const prismaMapper = new PrismaMapper();
  ...
  toFindAllQueryClean(query: FindAllPluginsQuery): FindAllPluginsQuery {
    if (!query) {
      return null;
    }
    return {
  ...
  toModel(app: PluginEntity) {
    return app && new PluginModel(app);
  }
  toModels(apps: PluginEntity[]) {
